...
import { 
  ...
  Pause, SkipBack, SkipForward, Search, Cloud
} from 'lucide-react';
...
const calculateOptimalPath = (startId, endId, floodLevels, vehicleType) => {
  ...
  while (queue.length > 0) {
    ...
    for (const neighborId of neighbors) {
      ...
      if (isPassable) {
        // Logika Path Breakdown (hanya untuk jalur sukses)
        const pathLog = {
            from: current.id,
            to: neighborId,
            baseDist: dist,
...
const App = () => {
  ...
  // NEW: State untuk Real Flood Data
  const [isLoadingRealData, setIsLoadingRealData] = useState(false);
  const [lastUpdateTime, setLastUpdateTime] = useState('');
  const [dataSource, setDataSource] = useState('manual'); // 'manual', 'real', 'simulation'
  
  // State Kalkulasi & Playback Algoritma
  ...
  // NEW: Fungsi untuk mengambil data banjir real dari API
  const fetchRealFloodData = async () => {
    ...
    try {
      ...
      // Panggil service untuk mendapatkan data real
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-4 md:p-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            ...
            {/* 2. UPDATED: Live Data Control with Real API */}
            <div className="md:col-span-1">
               ...
               <div className={`bg-white p-4 rounded-xl shadow-sm border border-slate-200 transition-all ${algoStepIndex !== -1 ? 'opacity-50 pointer-events-none' : ''}`}>
                  ...
                  {selectedNodeId ? (
                    ...
                    <div className="text-center py-6 text-slate-400 bg-slate-50 rounded-lg border border-dashed border-slate-300">
                      ...
                      <p className="text-xs">Klik titik pada peta untuk mengatur level banjir secara manual.</p>
                    </div>
                  )}
               </div>

               {/* 2. Ringkasan Singkat & Playback Control */}
...
const MapView = ({ locations, connections, floodLevels, selectedNodeId, setSelectedNodeId, pathResult, getNodeColor, algoStepIndex, algoLogs, dataSource }) => {
  const viewBoxW = 900;
  const viewBoxH = 550;
...
//   ChevronRight, Info, CheckCircle2, XCircle,
//   Wifi, RefreshCw, Sliders, Play, Zap, List,
//   Pause, SkipBack, SkipForward, Search
// } from 'lucide-react';
// import { floodDataService } from './services/floodDataService';
// import toast, { Toaster } from 'react-hot-toast';
...
//   // State Kalkulasi & Playback Algoritma
//   const [calculationResult, setCalculationResult] = useState({ path: [], cost: 0, logs: [], allExplorations: [] });
//   const [algoStepIndex, setAlgoStepIndex] = useState(-1); // -1 means playback off, show final result
//   const [isAlgoPlaying, setIsAlgoPlaying] = useState(false);
//   const playbackTimerRef = useRef(null);
...
//     const result = calculateOptimalPath(startNode, endNode, floodLevels, vehicleType);
//     setCalculationResult(result);
//     setAlgoStepIndex(-1); // Reset playback view to final result
//     setIsAlgoPlaying(false);
//   };
...
//             <div className="md:col-span-1">
//               <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block flex items-center gap-1">
//                 <Wifi size={12}/> Live Data Source
//               </label>
//               <button 
//                 onClick={simulateLiveApi}
//                 disabled={isSimulatingData}
//                 className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-lg bg-slate-800 text-white hover:bg-slate-700 disabled:opacity-50 transition-all text-sm font-medium"
//               >
//                 {isSimulatingData ? <RefreshCw className="animate-spin" size={16}/> : <Zap size={16}/>}
//                 {isSimulatingData ? 'Updating...' : 'Ambil Data Live'}
//               </button>
//             </div>
...
//                {/* 1. Panel Manual Slider (Kondisi: Tidak sedang Playback) */}
//                <div className={`bg-white p-4 rounded-xl shadow-sm border border-slate-200 transition-all ${algoStepIndex !== -1 ? 'opacity-50 pointer-events-none' : ''}`}>
//                   <div className="flex items-center gap-2 mb-4 text-slate-700">
//                     <Sliders size={18} />
//                     <h3 className="font-bold">Manual Intensity</h3>
...
//                   </h4>
//                   <div className="space-y-1">
//                     <div className="font-semibold text-slate-500 mb-1 text-[10px] uppercase">Kondisi Aman (Per Zona)</div>
//                     {/* <div className="grid grid-cols-2 gap-1">
//                       <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-cyan-500"></div> Utara</div>
//                       <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-amber-500"></div> Barat</div>
//                       <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-violet-500"></div> Pusat</div>
//                       <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-500"></div> Timur</div>
//                       <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-rose-500"></div> Selatan</div>
//                     </div> */}
                    
//                     <div className="font-semibold text-slate-500 mt-3 mb-1 text-[10px] uppercase">Simulasi</div>
//                     <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full border-2 border-yellow-500 bg-transparent"></div> Sedang Dicek</div>
//                     <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-100 border border-red-500"></div> Blocked/Pruned</div>
//                   </div>
//                </div>
...
// // --- SUB-COMPONENT: MAP VISUALIZATION ---
// const MapView = ({ locations, connections, floodLevels, selectedNodeId, setSelectedNodeId, pathResult, getNodeColor, algoStepIndex, algoLogs }) => {
//   const viewBoxW = 900;
//   const viewBoxH = 550;

//   // Determine active visualization state based on playback
//   const isPlaybackActive = algoStepIndex !== -1;
//   const currentLog = isPlaybackActive ? algoLogs[algoStepIndex] : null;

//   // Helper to determine edge style during playback or final result
//   const getEdgeStyle = (id1, id2) => {
//     // 1. Playback Mode
//     if (isPlaybackActive && currentLog) {
...
//       if (currentLog.type === 'CHECK_NEIGHBOR') {
//         if ((currentLog.from === id1 && currentLog.to === id2) || (currentLog.from === id2 && currentLog.to === id1)) {
//           if (currentLog.status === 'BLOCKED') return { stroke: '#ef4444', width: 4, dash: '5,5' }; // Red dashed
//           return { stroke: '#fbbf24', width: 4, dash: 'none' }; // Yellow solid (checking)
//         }
//       }
//       return { stroke: '#e2e8f0', width: 2, dash: 'none' }; // Gray default
//     }
...
//                   Menggunakan style transformOrigin: 'center' dan transformBox: 'fill-box'
//                   agar animasi ping tetap di tengah node (tidak lari).
//               */}
//               {isSelected && (
//                  <circle 
//                    cx={loc.x} cy={loc.y} r="28" 
